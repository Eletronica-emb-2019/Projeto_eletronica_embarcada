...
#include <Projeto.h> // header contendo funções em assembly (delay de 1ms e conversor ad)
...
static void Send_Float (float data)
{
    ...
    sign=(int)data;
    pint =(int)data;
    pfract= (int)((data - pint)*100); // remove parte inteira e multiplica fracionária por 100 para se ter 2 casas de precisão
    pint = abs(pint); // tira qualquer valor negativo da parte inteira
    pfract = abs(pfract); // tira qualquer valor negativo da parte fracionária
    if (sign < 0) // caso seja um valor negativo, manda o sinal de menos
    {
        Send_String("-");
    }
    Send_Int(pint);
...
void abertura (void){ // FUNÇÃO PARA ATIVAÇÃO DO MOTOR EM SENTIDO HORÁRIO, OU ABERTURA DA VÁLVULA
          ...
          cond1 = 1; // ATIVA COND1 QUE INDICA QUE A VÁLVULA ESTÁ ABERTA
             for (a = 0; a <2048; a++){ // ENQUANTO NÃO SEJA REPETIDO O SEGUINTE PROCESSO 2048 VEZES (UMA VOLTA COMPLETA)
                 ...
                 delay(2); //DELAY DA FUNÇÃO EM ASSEMBLY, DE 2* 1ms PODENDO SER ALTERADO PARA MODIFICAR VELOCIDADE E TORQUE DO MOTOR
                 }
